...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash('Account already existed. Try Login!')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(email=email).first()
        if not user:
            flash('No user found with this email. Please Signup first or try again with different email.')
...
def show_post(post_id):
    ...
    if request.method == 'POST':
        if not current_user.is_authenticated:
            flash('You must login first to comment on a post')
            return redirect(url_for('login'))
        comment_text = Comment(text=form.comment.data, comment_author=current_user, parent_post=requested_post)
